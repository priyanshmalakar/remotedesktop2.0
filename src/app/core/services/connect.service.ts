...
export class ConnectService {
    ...
    sendScreenSize() {
        ...
        this.socketService.sendMessage(
            `screenSize,${finalWidth},${finalHeight}`
        );
    }
    ...
    async videoConnector() {
        ...
        console.log(
            '[CONNECT] 🎥 Camera/mic will start when user enables them'
        );
        ...
        console.log(
            '[CONNECT] ✅ Screen tracks added. Camera will be added when enabled by user.'
        );
        ...
        this.peer1.on('data', async data => {
            if (data) {
                try {
                    ...
                    if (fileTransfer.startsWith('disable-host-mouse:')) {
                const shouldDisable = fileTransfer.split(':')[1] === 'true';
                
                if (shouldDisable) {
                    this.connectHelperService.startBlockingHostMouse();
                } else {
                    this.connectHelperService.stopBlockingHostMouse();
                }
                
                console.log('[CONNECT] 🖱️ Host mouse blocked:', shouldDisable);
                return;
            }
    ...
    async startLocalCamera() {
        try {
            ...
            if (!this.hostCameraEnabled && !this.hostMicEnabled) {
                console.log(
                    '[CONNECT] ⚠️ Both camera and mic are disabled, skipping getUserMedia'
                );
                return null;
            }
   ...
   async generateId() {
    ...
    buttons: [
        ...
        try {
            ...
            console.log(
                '[CONNECT] ✅ Navigated to /home, ID preserved:',
                this.id
            );
        } catch (err) {
    ...
    createDualChatUI() {
        // Remove the old dual chat implementation
        // Create only media control buttons
        if (!document.getElementById('mediaControlsBox')) {
            const controlsBox = document.createElement('div');
            controlsBox.id = 'mediaControlsBox';
            controlsBox.style.position = 'fixed';
            controlsBox.style.bottom = '20px';
            controlsBox.style.left = '50%';
            controlsBox.style.transform = 'translateX(-50%)';
            controlsBox.style.zIndex = '101';
            controlsBox.style.display = 'flex';
            controlsBox.style.gap = '12px';
            controlsBox.style.alignItems = 'center';
            controlsBox.style.background = 'rgba(0, 0, 0, 0.7)';
            controlsBox.style.padding = '12px 20px';
            controlsBox.style.borderRadius = '50px';
            controlsBox.style.backdropFilter = 'blur(10px)';
            controlsBox.style.border = '1px solid rgba(255, 255, 255, 0.1)';

            controlsBox.innerHTML = `
  <button id="startVideoCallBtn" style="
    padding: 12px 24px;
  ...
  <div id="videoCallControls" style="display: none; gap: 12px;">
    <button id="hostCameraToggle" style="
      width: 48px;
      height: 48px;
      background: #374151;
      border: none;
    ...
    <button id="endVideoCallBtn" style="
      padding: 12px 24px;
...
`;
            ...
            const videoCallControls =
                document.getElementById('videoCallControls');
            const endVideoBtn = document.getElementById('endVideoCallBtn');
            ...
            endVideoBtn?.addEventListener('click', async () => {
                ...
                if (this.hostCameraEnabled) await this.toggleHostCamera();
                if (this.hostMicEnabled) await this.toggleHostMic();
                ...
                hostMicBtn!.style.background = '#374151';
                hostMicBtn!.style.transform = 'scale(1)';
            ...
            hostMicBtn?.addEventListener('click', async () => {
                ...
                if (this.hostMicEnabled) {
                    hostMicBtn.style.background =
                        'linear-gradient(135deg, #06b6d4, #0891b2)';
                    hostMicBtn.style.transform = 'scale(1.05)';
                } else {
                    hostMicBtn.style.background = '#374151';
                    hostMicBtn.style.transform = 'scale(1)';
                }
            ...
            // Hover effects
            [hostCameraBtn, hostMicBtn, startVideoBtn, endVideoBtn].forEach(
                btn => {
                    ...
                    btn?.addEventListener('mouseleave', () => {
                        if (
                            !(
                                (btn.id === 'hostCameraToggle' &&
                                    this.hostCameraEnabled) ||
                                (btn.id === 'hostMicToggle' &&
                                    this.hostMicEnabled)
                            )
                        ) {
    ...
    addChatMessage(sender: string, msg: string, role: 'host' | 'user') {
        const containerId =
            role === 'host' ? 'hostChatMessages' : 'userChatMessages';
        ...
        if (sender.startsWith('You')) {
            div.innerHTML = `<div style="text-align:right;"><span style="display:inline-block; padding:6px 10px; background:#06b6d4; color:black; border-radius:10px; max-width:80%;">${this.escapeHtml(
                msg
            )}</span></div>`;
        } else {
            div.innerHTML = `<div style="text-align:left;"><span style="display:inline-block; padding:6px 10px; background:#222; color:white; border-radius:10px; max-width:80%;">${this.escapeHtml(
                msg
            )}</span><div style="font-size:11px;color:#9ca3af;margin-top:4px;">${this.escapeHtml(
                sender
            )}</div></div>`;
        }
    ...
    removeChatWindow() {
        const controlsBox = document.getElementById('mediaControlsBox');
