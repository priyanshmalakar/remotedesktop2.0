...
export class ConnectService {
    ...
    createDualChatUI() {
        ...
        if (!document.getElementById('mediaControlsBox')) {
            ...
            <button id="hostCameraToggle" 
                style="width: 50px; height: 50px; border-radius: 50%; border: none; cursor: pointer; 
                       background: #374151; color: white; font-size: 24px; display: flex; 
        ...
        `;
            ...
            // Add hover effects
            [hostCameraBtn, hostMicBtn].forEach(btn => {
                btn?.addEventListener('mouseenter', () => {
                    if (btn.style.background === '#374151') {
                ...
                btn?.addEventListener('mouseleave', () => {
                    if (
                        !(
                            ...
                            (btn.id === 'hostMicToggle' && this.hostMicEnabled)
                        )
                    ) {
