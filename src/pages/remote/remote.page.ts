...
@ViewChild('hostCameraVideo') hostCameraVideoRef: ElementRef<HTMLVideoElement>; // Add this
    ...
    async startVideoCall() {
        try {
            ...
            if (localVideo) {
                ...
                localVideo.play().catch(e => console.error('[REMOTE] Local video play error:', e));
            }

            console.log('[REMOTE] ✅ Local stream ready, will add to peer when connected');
...
initPeer(id) {
    ...
    if (this.localStream) {
        ...
        this.localStream.getTracks().forEach((track, index) => {
            console.log(`[REMOTE] 📤 Adding local track ${index}:`, track.kind, track.label);
...
startClipboardMonitoring() {
    if (!this.electronService.isElectron) {
        console.log('[REMOTE] ⚠️ Not in Electron, skipping clipboard monitoring');
        return;
    }
        ...
        clipboard
            .on('text-changed', () => {
                if (this.peer2 && this.connected) {
                    ...
                    console.log('[REMOTE] 📋 Clipboard text changed, sending to host');
...
updateVideoDisplays(receivedTracks: any) {
    ...
    if (receivedTracks.screenVideo) {
        ...
        if (remoteVideo && remoteVideo.srcObject !== screenStream) {
            ...
            remoteVideo.play()
                ...
                .catch(e => console.error('[REMOTE] ❌ Main video play error:', e));
        }

        // Update background video
        const videoBg: HTMLVideoElement = this.elementRef.nativeElement.querySelector('#videobg');
        if (videoBg && videoBg.srcObject !== screenStream) {
            videoBg.srcObject = screenStream;
            videoBg.play().catch(e => console.log('[REMOTE] Background video error (ok to ignore):', e));
        }
        ...
        if (video) {
            ...
            if (currentSrc !== screenStream) {
                ...
                video.play()
                    .then(() => console.log('[REMOTE] ✅ Control video playing'))
                    .catch(e => console.error('[REMOTE] ❌ Control video play error:', e));

                // Bind event listeners only once
                if (!this.boundMouseDownListener) {
                    console.log('[REMOTE] 🖱️ Binding mouse/keyboard listeners...');
                ...
                video.addEventListener('loadeddata', () => {
                    console.log('[REMOTE] 📏 Video loaded, calculating size...');
                    this.calcVideoSize();
                }, { once: true });

                video.addEventListener('resize', () => {
                    console.log('[REMOTE] 📐 Video resized:', video.videoWidth, 'x', video.videoHeight);
                    this.calcVideoSize();
                });
            }
        }
    ...
    if (receivedTracks.cameraVideo) {
        ...
        if (hostCameraVideo && hostCameraVideo.srcObject !== cameraStream) {
            ...
            hostCameraVideo.play()
                .then(() => console.log('[REMOTE] ✅ Host camera PiP playing'))
                .catch(e => console.error('[REMOTE] ❌ Host camera play error:', e));
    ...
    removeEventListeners() {
        if (this.video) {
            if (this.boundMouseDownListener)
                this.video.removeEventListener('mousedown', this.boundMouseDownListener);
            if (this.boundMouseUpListener)
                this.video.removeEventListener('mouseup', this.boundMouseUpListener);
            if (this.boundDblClickListener)
                this.video.removeEventListener('dblclick', this.boundDblClickListener);
            if (this.boundMouseMoveListener)
                this.video.removeEventListener('mousemove', this.boundMouseMoveListener);
        }
    }
